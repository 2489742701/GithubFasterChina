...
支持正版，反对软件倒卖行为！"""
    ...
    def setup_steam_content(self):
        ...
        self.steam_update_btn = ttk.Button(status_frame, text="立即更新Steam Hosts", 
                                          command=self.update_steam_hosts, state="normal")
        self.steam_update_btn.pack(fill=tk.X, pady=(10, 0))
        ...
        info_text = """• 此功能使用 Clov614/SteamHostSync 项目的hosts配置
...
• 如果遇到问题，可以使用紧急恢复功能"""
    ...
    def show_steam_content(self):
        ...
        # 不自动加载，用户需要点击按钮手动更新
        self.steam_status_label.config(text="请点击'刷新配置'按钮来更新")
        self.steam_update_btn.config(state="normal")
    
    def load_steam_hosts_data(self):
        """加载Steam hosts数据 - 使用重试机制"""
        try:
            self.steam_status_label.config(text="正在获取Steam hosts配置...")
            self.steam_update_btn.config(state="disabled")
            
            url = "https://raw.githubusercontent.com/Clov614/SteamHostSync/main/hosts"
            # 使用带重试机制的网络请求
            try:
                with self.fetch_with_retry(url) as response:
                    ...
                    self.steam_status_label.config(text="已获取最新Steam hosts配置")
                    self.steam_update_btn.config(state="normal")
...
# 更新时间: {}\n\n""".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def show_dns_helper(self):
        ...
        dns_window.geometry("600x7")  # 进一步增加窗口高度以完全显示所有内容
        ...
        try:
            ...
            for target in targets:
                ...
                result = subprocess.run(['ping', param, '3', target], 
                                      capture_output=True, text=True)
                status = "通" if result.returncode == 0 else "不通"
                results.append(f"{target}: {status}")
            
            messagebox.showinfo("网络诊断结果", "\n".join(results))
